 ...
 *
 * Target:    ESP32-S3
 * Framework: Arduino (ESP32 Arduino Core 3.x / ESP-IDF 5.x)
 */
...
void setupOutputPins() {
    ...
    if (config.pin_led >= 0)      { pinMode(config.pin_led, OUTPUT); digitalWrite(config.pin_led, LOW); }
    if (config.pin_bt_led >= 0)   { pinMode(config.pin_bt_led, OUTPUT); digitalWrite(config.pin_bt_led, LOW); }
}
...
static const uint8_t hid_to_ascii_lower[104] = {
    0x00,0x00,0x00,0x00, 'a','b','c','d', 'e','f','g','h',
    'i','j','k','l', 'm','n','o','p', 'q','r','s','t',
    'u','v','w','x', 'y','z',
    '1','2','3','4', '5','6','7','8', '9','0',
    0x0D,0x1B,0x08,0x09, 0x20,'-','=','[', ']','\\',0x00,';',
    '\'','`',',', '.','/','0x00',
...
const char* getSpecialSequence(uint8_t keycode) {
    for (int i = 0; i < config.num_special_keys; i++) {
        if (config.special_keys[i].enabled &&
            config.special_keys[i].hid_keycode == keycode) {
            return config.ansi_mode ?
                config.special_keys[i].seq_ansi :
                config.special_keys[i].seq_wyse;
        }
    }
    return NULL;
}
...
void processKeypress(uint8_t keycode, uint8_t modifiers) {
    ...
    uint8_t ascii = effective_shift ?
        hid_to_ascii_upper[keycode] :
    ...
    if (ctrl) {
        ...
        else if (ascii >= '[' && ascii <= '_')   ascii = ascii - '@';
        else if (ascii == '@' || ascii == ' ')   ascii = 0x00;
...
void processHidReport(const KeyReport *report) {
    ...
    for (int i = 0; i < 6; i++) {
        ...
        bool isNew = true;
        for (int j = 0; j < 6; j++) {
            if (keys[i] == prevKeys[j]) { isNew = false; break; }
        }
        if (isNew) {
    ...
    if (currentKey != 0) {
        ...
        for (int i = 0; i < 6; i++) {
            if (keys[i] == currentKey) { stillHeld = true; break; }
        }
        if (!stillHeld) { currentKey = 0; repeating = false; }
        else currentMods = modifiers;
    }
...
void handleAutoRepeat() {
    ...
    if (!repeating) {
        if (now - keyDownTime >= config.repeat_delay_ms) {
            repeating = true; lastRepeatTime = now;
...
static void usb_client_event_cb(const usb_host_client_event_msg_t *msg, void *arg) {
    switch (msg->event) {
        case USB_HOST_CLIENT_EVENT_NEW_DEV:
            xSemaphoreGive(usb_device_sem); break;
        case USB_HOST_CLIENT_EVENT_DEV_GONE:
            usb_keyboard_connected = false;
            if (usb_dev_hdl) { usb_host_device_close(usb_client_hdl, usb_dev_hdl); usb_dev_hdl = NULL; }
            logKey("[USB] Disconnected"); break;
        default: break;
    }
}
...
static void usb_set_boot_protocol(usb_device_handle_t dev, uint8_t iface) {
    ...
    ctrl->num_bytes = 8;
    ctrl->data_buffer[0] = 0x21; ctrl->data_buffer[1] = 0x0B;
    ctrl->data_buffer[2] = 0x00; ctrl->data_buffer[3] = 0x00;
    ctrl->data_buffer[4] = iface; ctrl->data_buffer[5] = 0x00;
    ctrl->data_buffer[6] = 0x00; ctrl->data_buffer[7] = 0x00;
    ctrl->device_handle = dev; ctrl->bEndpointAddress = 0x00;
    ...
    for (int i = 0; i < 50 && !done; i++) vTaskDelay(pdMS_TO_TICKS(10));
...
static void usb_host_daemon_task(void *arg) {
    ...
    while (true) usb_host_lib_handle_events(portMAX_DELAY, NULL);
}

static void usb_keyboard_task(void *arg) {
    usb_host_client_config_t cfg = { .is_synchronous = false, .max_num_event_msg = 5,
    ...
    while (true) {
        ...
        if (!usb_keyboard_connected && xSemaphoreTake(usb_device_sem, 0) == pdTRUE) {
            ...
            uint8_t iface = 0; bool in_kbd = false, connected = false;
            ...
            while (off < total && !connected) {
                ...
                if (dtype == 0x05 && in_kbd && dlen >= 7 && (p[off+2] & 0x80)) {
                    ...
                    usb_xfer_in->num_bytes = 8; usb_xfer_in->timeout_ms = 0;
                    usb_keyboard_connected = true; connected = true;
                    logKey("[USB] Keyboard connected");
                    usb_host_transfer_submit(usb_xfer_in);
                }
                off += dlen;
            }
            if (!connected) { usb_host_device_close(usb_client_hdl, usb_dev_hdl); usb_dev_hdl = NULL; }
        }
    }
}
...
static void bt_scan_task(void *arg) {
    while (true) {
        if (!bt_scan_requested) { vTaskDelay(pdMS_TO_TICKS(100)); continue; }
        ...
        logKey("[BT] Scanning...");
        size_t num = 0; esp_hid_scan_result_t *results = NULL;
        ...
        if (num > 0) {
            ...
            esp_hid_scan_result_free(results);
        } else {
...
void startBluetooth() {
    esp_bt_mode_t mode;
    if (config.enable_bt_classic && config.enable_ble) mode = ESP_BT_MODE_BTDM;
    else if (config.enable_bt_classic) { esp_bt_controller_mem_release(ESP_BT_MODE_BLE); mode = ESP_BT_MODE_CLASSIC_BT; }
    else { esp_bt_controller_mem_release(ESP_BT_MODE_CLASSIC_BT); mode = ESP_BT_MODE_BLE; }
    ...
    esp_bt_dev_set_device_name("Wyse-Adapter");
...
void startWebServer() {
    ...
    server.on("/api/preset/wyse50", HTTP_GET, []() {
        ...
        for (int i = 0; i < tmp.num_special_keys; i++) {
            ...
            k["wyse_hex"] = seqToHex(tmp.special_keys[i].seq_wyse);
            k["ansi_hex"] = seqToHex(tmp.special_keys[i].seq_ansi);
            k["wyse_display"] = seqToReadable(tmp.special_keys[i].seq_wyse);
    ...
    server.on("/api/preset/vt100", HTTP_GET, []() {
        ...
        String out = "{\"special_keys\":["
            "{\"keycode\":58,\"label\":\"F1\",\"wyse_hex\":\"1b4f50\",\"ansi_hex\":\"1b4f50\",\"wyse_display\":\"ESC O P\",\"ansi_display\":\"ESC O P\",\"enabled\":true},"
            "{\"keycode\":59,\"label\":\"F2\",\"wyse_hex\":\"1b4f51\",\"ansi_hex\":\"1b4f51\",\"wyse_display\":\"ESC O Q\",\"ansi_display\":\"ESC O Q\",\"enabled\":true},"
            "{\"keycode\":60,\"label\":\"F3\",\"wyse_hex\":\"1b4f52\",\"ansi_hex\":\"1b4f52\",\"wyse_display\":\"ESC O R\",\"ansi_display\":\"ESC O R\",\"enabled\":true},"
            "{\"keycode\":61,\"label\":\"F4\",\"wyse_hex\":\"1b4f53\",\"ansi_hex\":\"1b4f53\",\"wyse_display\":\"ESC O S\",\"ansi_display\":\"ESC O S\",\"enabled\":true},"
            "{\"keycode\":82,\"label\":\"Up\",\"wyse_hex\":\"1b5b41\",\"ansi_hex\":\"1b5b41\",\"wyse_display\":\"ESC [ A\",\"ansi_display\":\"ESC [ A\",\"enabled\":true},"
            "{\"keycode\":81,\"label\":\"Down\",\"wyse_hex\":\"1b5b42\",\"ansi_hex\":\"1b5b42\",\"wyse_display\":\"ESC [ B\",\"ansi_display\":\"ESC [ B\",\"enabled\":true},"
            "{\"keycode\":79,\"label\":\"Right\",\"wyse_hex\":\"1b5b43\",\"ansi_hex\":\"1b5b43\",\"wyse_display\":\"ESC [ C\",\"ansi_display\":\"ESC [ C\",\"enabled\":true},"
            "{\"keycode\":80,\"label\":\"Left\",\"wyse_hex\":\"1b5b44\",\"ansi_hex\":\"1b5b44\",\"wyse_display\":\"ESC [ D\",\"ansi_display\":\"ESC [ D\",\"enabled\":true},"
            "{\"keycode\":74,\"label\":\"Home\",\"wyse_hex\":\"1b5b48\",\"ansi_hex\":\"1b5b48\",\"wyse_display\":\"ESC [ H\",\"ansi_display\":\"ESC [ H\",\"enabled\":true}"
            "]}";
        server.send(200, "application/json", out);
    });
    ...
    server.on("/api/preset/adm3a", HTTP_GET, []() {
        ...
        String out = "{\"special_keys\":["
            "{\"keycode\":82,\"label\":\"Up\",\"wyse_hex\":\"0b\",\"ansi_hex\":\"0b\",\"wyse_display\":\"^K\",\"ansi_display\":\"^K\",\"enabled\":true},"
            "{\"keycode\":81,\"label\":\"Down\",\"wyse_hex\":\"0a\",\"ansi_hex\":\"0a\",\"wyse_display\":\"^J\",\"ansi_display\":\"^J\",\"enabled\":true},"
            "{\"keycode\":79,\"label\":\"Right\",\"wyse_hex\":\"0c\",\"ansi_hex\":\"0c\",\"wyse_display\":\"^L\",\"ansi_display\":\"^L\",\"enabled\":true},"
            "{\"keycode\":80,\"label\":\"Left\",\"wyse_hex\":\"08\",\"ansi_hex\":\"08\",\"wyse_display\":\"^H\",\"ansi_display\":\"^H\",\"enabled\":true},"
            "{\"keycode\":74,\"label\":\"Home\",\"wyse_hex\":\"1e\",\"ansi_hex\":\"1e\",\"wyse_display\":\"^^\",\"ansi_display\":\"^^\",\"enabled\":true}"
            "]}";
        server.send(200, "application/json", out);
    });
...
void handlePairButton() {
    ...
    if (state == LOW && pairBtnLastState == HIGH) {
        pairBtnDownTime = millis(); pairBtnTriggered = false;
    }
...
void checkModeJumper() {
    ...
    if (newMode != config.ansi_mode) {
        ...
        logKey("Mode: %s (jumper)", config.ansi_mode ? "ANSI" : "Wyse");
    }
}
...
void setup() {
    ...
    ESP_LOGI(TAG, "========================================");
    ESP_LOGI(TAG, " Wyse Keyboard Adapter  v4.0");
    ESP_LOGI(TAG, " Web-configurable | USB + BT + BLE");
    ESP_LOGI(TAG, "----------------------------------------");
    ESP_LOGI(TAG, " Mode:    %s", config.ansi_mode ? "ANSI/VT100" : "Wyse Native");
...
void loop() {
    ...
    if (++modeCounter >= 100) { modeCounter = 0; checkModeJumper(); }
